...
export function JoinField({ label, children, icon, className }: Props) {
  return (
    <label className={cn("flex w-full flex-col gap-1.5", className)}>
      ...
      <span className="relative flex min-h-12 items-center gap-3 rounded-[var(--radius-md)] border border-[var(--ink-600)] bg-[var(--ink-800)] px-3.5 transition-[border-color] duration-[var(--dur-fast)] focus-within:border-[var(--key-500)]/55">
        {icon ? (
